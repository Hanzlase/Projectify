// Runs meeting reminder processing directly in-process using Prisma + Gmail SMTP.
// No HTTP polling — avoids 405 errors and the "route not compiled yet" race condition.

const { PrismaClient } = require('@prisma/client');
const nodemailer = require('nodemailer');

// Increase interval to reduce DB churn (especially on free-tier DBs / small pools)
...
function buildDatabaseUrl() {
  ...
  // Match lib/prisma.ts dev mitigation
  if (!isProd) {
    const extra = 'connection_limit=10&pool_timeout=30';
...
function getTransporter() {
  ...
  if (!user || !pass) {
    throw new Error('Gmail SMTP not configured for scheduler. Set GMAIL_SMTP_USER and GMAIL_SMTP_PASS in .env');
  }
  ...
  transporter.__from = from;
  return transporter;
}

async function sendSmtpEmail({ to, subject, html, text }) {
  const t = getTransporter();
  const from = t.__from || process.env.EMAIL_FROM || process.env.GMAIL_SMTP_USER;
  return t.sendMail({
...
async function sendReminderEmail({ to, userName, meetingTitle, scheduledAt, duration, meetingLink, description, groupName, reminderType }) {
  ...
  try {
    ...
    console.log(`[Meeting Scheduler] Email sent to ${to}`);
  } catch (err) {
...
async function processReminders() {
  ...
  try {
    ...
    for (const reminder of dueReminders) {
      ...
      // Send sequentially to reduce DB + SMTP pressure (and avoid Prisma pool exhaustion)
    ...
    consecutiveErrors++;
    console.error('[Meeting Scheduler] processReminders error:', err?.message || err);

    // On repeated pool timeouts, wait a bit longer before next run
    if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
      console.warn(`[Meeting Scheduler] ${consecutiveErrors} consecutive errors; will reduce load automatically (next cycles may be slower).`);
    }
  } finally {
    isProcessing = false;
  }
...
function startMeetingReminderScheduler() {
  console.log(`[Meeting Scheduler] Started — processing reminders every ${Math.round(POLL_INTERVAL_MS / 1000)}s`);
  // First run after a short delay to let Next.js finish booting
  setTimeout(processReminders, INITIAL_DELAY_MS);

  setInterval(() => {
    // Basic jitter so restarts don't cause thundering herds on DB
    const jitter = Math.floor(Math.random() * 5_000);
    setTimeout(processReminders, jitter);
  }, POLL_INTERVAL_MS);
}
