// Runs meeting reminder processing directly in-process using Prisma + Brevo HTTP API.
// No SMTP — all email requests go over HTTPS port 443 (works on Railway).
...
async function sendBrevoEmail({ to, subject, html, text }) {
  ...
  const senderEmail = process.env.BREVO_SENDER_EMAIL || 'FypSync@gmail.com';
