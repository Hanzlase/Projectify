            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-zinc-500">{projects.length} idea{projects.length !== 1 ? 's' : ''}</p>
                </div>
              </div>
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl font-medium transition-all ${
                        filter === 'my'
                          ...
                          : 'bg-gray-100 dark:bg-zinc-700 text-gray-600 hover:bg-gray-200'
                      }`}
                    >
                      My Ideas
                    </button>
                  ...
                  <div className="flex-1 flex gap-2">
                    <div className="relative">
                      <select
                        ...
                        className="h-10 px-4 pr-8 border border-gray-200 dark:border-zinc-700 rounded-xl text-sm bg-white dark:bg-zinc-700 dark:text-[#E4E4E7] appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#1a5d1a]/20"
                      >
                    ...
                    <Button onClick={handleSearch} variant="outline" className="border-gray-200 hover:bg-gray-50 dark:bg-zinc-700/50">
                      ...
                      <div className="h-32 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-zinc-700 dark:to-zinc-800 relative overflow-hidden">
                        ...
                        {isOwner && (
                          <div className="absolute bottom-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={(e) => { e.stopPropagation(); openEditModal(project); }} className="w-8 h-8 bg-white/90 dark:bg-[#27272A]/90 backdrop-blur-sm rounded-full flex items-center justify-center text-gray-600 dark:text-zinc-300 hover:text-[#1a5d1a] hover:bg-white dark:hover:bg-[#27272A] transition-colors shadow-md">
                              <Edit className="w-4 h-4" />
                            </button>
                            <button onClick={(e) => { e.stopPropagation(); openDeleteModal(project); }} disabled={deleting === project.projectId} className="w-8 h-8 bg-white/90 dark:bg-[#27272A]/90 backdrop-blur-sm rounded-full flex items-center justify-center text-gray-600 dark:text-zinc-300 hover:text-red-600 hover:bg-white dark:hover:bg-[#27272A] transition-colors shadow-md">
                      ...
                      <CardContent className="p-3">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-zinc-800">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-gray-500 dark:text-zinc-500">{project.creator?.name}</span>
                          </div>
      ...
      <AnimatePresence>
        {(showCreateModal || showEditModal) && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => { setShowCreateModal(false); setShowEditModal(false); }}>
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="bg-white dark:bg-[#27272A] rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="sticky top-0 bg-white dark:bg-[#27272A] border-b border-gray-200 dark:border-zinc-700 px-6 py-4 flex items-center justify-between z-10">
                <h2 className="text-xl font-bold text-gray-900 dark:text-[#E4E4E7]">{showEditModal ? 'Edit Idea' : 'Add New Idea'}</h2>
                <button onClick={() => { setShowCreateModal(false); setShowEditModal(false); }} className="w-8 h-8 rounded-full bg-gray-100 dark:bg-zinc-700 hover:bg-gray-200 flex items-center justify-center transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    <button type="button" onClick={() => setFormData(prev => ({ ...prev, visibility: 'private' }))} className={`flex-1 py-3 px-4 rounded-xl border-2 transition-all flex items-center justify-center gap-2 ${formData.visibility === 'private' ? 'border-[#1a5d1a] bg-[#d1e7d1] text-[#1a5d1a]' : 'border-gray-200 dark:border-zinc-600 text-gray-600 dark:text-zinc-400 hover:border-gray-300'}`}>
                      <Lock className="w-4 h-4" />
                      Private
                    </button>
                    <button type="button" onClick={() => setFormData(prev => ({ ...prev, visibility: 'public' }))} className={`flex-1 py-3 px-4 rounded-xl border-2 transition-all flex items-center justify-center gap-2 ${formData.visibility === 'public' ? 'border-[#1a5d1a] bg-[#d1e7d1] text-[#1a5d1a]' : 'border-gray-200 dark:border-zinc-600 text-gray-600 dark:text-zinc-400 hover:border-gray-300'}`}>
                      <Globe className="w-4 h-4" />
                      Public
                    </button>
                  </div>
                ...
                <div>
                  ...
                  {formData.thumbnailUrl ? (
                    ...
                    <button type="button" onClick={() => thumbnailInputRef.current?.click()} disabled={uploadingThumbnail} className="w-full h-32 border-2 border-dashed border-gray-300 rounded-xl hover:border-[#1a5d1a] transition-colors flex flex-col items-center justify-center gap-2 text-gray-500 dark:text-zinc-500">
                ...
                {/* Document Upload */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1.5">Project Document <span className="text-red-500">*</span></label>
                  ...
                  {formData.documentUrl ? (
                    <div className="p-4 border border-gray-200 rounded-xl bg-gray-50 dark:bg-zinc-700/50 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-[#d1e7d1] rounded-lg flex items-center justify-center">
                          <FileText className="w-5 h-5 text-[#1a5d1a]" />
                        </div>
                        <div>
                          <p className="text-sm font-medium text-gray-900 dark:text-[#E4E4E7]">{formData.documentName}</p>
                          <p className="text-xs text-gray-500 dark:text-zinc-500">Document uploaded</p>
                        </div>
                      </div>
                    ...
                    <button type="button" onClick={() => documentInputRef.current?.click()} disabled={uploadingDocument} className="w-full p-6 border-2 border-dashed border-gray-300 rounded-xl hover:border-[#1a5d1a] transition-colors flex flex-col items-center justify-center gap-2 text-gray-500 dark:text-zinc-500">
                      {uploadingDocument ? <Loader2 className="w-6 h-6 animate-spin" /> : <>
                        <Upload className="w-8 h-8" />
                        <span className="text-sm">Upload PDF or DOCX file</span>
                        <span className="text-xs text-gray-400 dark:text-zinc-500">Max 10MB</span>
                      </>}
                    </button>
                  )}
                ...
                {showEditModal && (
                  <>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1.5">Title</label>
                      <Input value={formData.title} onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))} placeholder="Project title" className="h-11 border-gray-200 dark:border-zinc-700 focus:border-[#1a5d1a] focus:ring-[#1a5d1a]/20 rounded-xl" />
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1.5">Description</label>
                      <textarea value={formData.description} onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))} placeholder="Project description" rows={3} className="w-full px-4 py-3 border border-gray-200 dark:border-zinc-700 dark:bg-zinc-700 dark:text-[#E4E4E7] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#1a5d1a]/20 focus:border-[#1a5d1a] resize-none" />
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1.5">Category</label>
                      <div className="relative">
                        <select value={formData.category} onChange={(e) => setFormData(prev => ({ ...prev, category: e.target.value }))} className="w-full h-11 px-4 pr-8 border border-gray-200 dark:border-zinc-700 rounded-xl text-sm bg-white dark:bg-zinc-700 dark:text-[#E4E4E7] appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#1a5d1a]/20 focus:border-[#1a5d1a]">
              ...
              <div className="sticky bottom-0 bg-white dark:bg-[#27272A] border-t border-gray-200 dark:border-zinc-700 px-6 py-4 flex gap-3">
      ...
      <AnimatePresence>
        {showDeleteModal && projectToDelete && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={closeDeleteModal}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white dark:bg-[#27272A] rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
              <div className="text-center mb-6">
                <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Trash2 className="w-8 h-8 text-red-600" />
                </div>
                <h3 className="text-xl font-bold text-gray-900 dark:text-[#E4E4E7] mb-2">Delete Project?</h3>
                <p className="text-gray-500 dark:text-zinc-500">Are you sure you want to delete "{projectToDelete.title}"? This action cannot be undone.</p>
              </div>
              <div className="flex gap-3">
