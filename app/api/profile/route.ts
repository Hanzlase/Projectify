...
import { getActivePhase, getSupervisorGroups } from "@/lib/cohort-utils";
...
async function getOtherUserProfile(userId: number, role: string) {
  try {
    if (role === 'supervisor') {
      ...
      const { activeGroups, pastProjects } = await getSupervisorGroups(supervisor.userId);
      ...
      return NextResponse.json({
        ...
        totalGroups,
        available: totalGroups < supervisor.maxGroups,
        campus: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (user.role === "student" && user.student) {
      ...
      const { activeGroups, pastProjects } = await getSupervisorGroups(user.supervisor.userId);
      const totalGroups = activeGroups.length;
      profileData = {
        ...profileData,
